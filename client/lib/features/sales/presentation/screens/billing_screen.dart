...
class _CartItem {
  ...
  double quantity;
  double get subtotal => stockItem.sellPrice * quantity;
  double get profit => (stockItem.sellPrice - stockItem.buyPrice) * quantity;
...
class _BillingScreenState extends ConsumerState<BillingScreen> {
  ...
  Future<void> _processSale(AppLocalizations l10n) async {
    ...
    try {
      final items = _cart
          .map((item) => (itemId: item.stockItem.id, quantity: item.quantity))
          .toList();

      await ref.read(salesProvider.notifier).processCart(items);
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Rs. ${cartItem.stockItem.sellPrice.toStringAsFixed(0)} × ${cartItem.quantity.toStringAsFixed(cartItem.quantity == cartItem.quantity.roundToDouble() ? 0 : 1)}',
                  style: GoogleFonts.inter(
                    fontSize: 13,
